...
class Booking(models.Model):
    ...
    status = fields.Selection([
        ...
        ('check_out', 'Check-out'),
        ('cancelled', 'Cancelled'),
    ], default='booked', string="Booking Status")
    ...
    #Available room 
    available_room_ids = fields.Many2many("room", compute="_compute_available_rooms", store=True)
    ...
    def _get_available_rooms(self, check_in_date, check_out_date):
        print("\n\n-------------------- METHODDDDDD")
        overlap_bookings = self.env["booking"].search([
        ...
        available_rooms = self.env["room"].search([
    ...
    def _check_room_availability(self):
        for record in self:
            print("\n\n\n\n>>>>>>>>>>>>>DATE..", record.check_out_date < record.check_in_date)
            print("\n\n\n\n>>...........DATE..", record.check_out_date <= record.check_in_date)
            if record.check_out_date < record.check_in_date:
                raise ValidationError("Check-out date (big)greater than check-in date!!")
            

            # Check bookings for the same room
            date_bookings = self.env["booking"].search([
            ...
            if date_bookings:
                raise ValidationError(f"Room {record.room_id.room_number} is already booked during the selected dates!")
    ...
    extra_charges_total = fields.Float(compute="_compute_extra_charges_total", store=True)
    ...
    def _compute_total_price(self):
        for record in self:
            if record.room_id and record.check_in_date and record.check_out_date:
                ...
                record.total_price = room_cost + record.extra_person_cost + record.extra_charges_total
    ...
    def _check_max_person_limit(self):
        for record in self:
            ...
            if record.num_guests > (record.max_persons + 2):  # Allow 2 extra guest max
                raise ValidationError(f"Maximum allowed guests for this room is {record.max_persons + 2}!")